...
exports.list = function (req, reply) {
  ...
  drive.files.list({
    ...
    response.files.forEach((image) => {
      ...
      const matches = /^([0-9]+)\.\s+(.*)/.exec(image.basename);

      if (matches && matches.length > 2) {
        image.title = matches[2];
        image.sortOrder = parseFloat(matches[1], 10);
      }
      else {
        image.title = image.basename;
        image.sortOrder = response.files.length;
      }
    });
